...
def plot_submission(inputs: torch.Tensor, predictions: torch.Tensor, log_path: str, update: int) -> None:
    """
    Save a image of the plot of the inputs, targets and predictions to path.
    ...
    for i in range(len(inputs)):
        ...
        ax3.clear()
        ax3.set_axis_off()
        ax3.set_title("prediction + input")
        input_arr = inputs[i, 0].copy()
        input_arr[input_arr == 0] = prediction
        ax3.imshow(input_arr, cmap="gray")
